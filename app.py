...
def upload_rep():
    # Proteção: Se não tiver ninguém logado, chuta de volta pro login
    if 'representante_logado' not in session:
        return redirect(url_for('login_rep'))
    
    turma_atual = session['representante_logado']
    return render_template('upload_rep.html', turma=turma_atual)
...
def exibir_turma(id_turma):
    conn = conectar_banco()
    # Aqui está o segredo: filtramos pela coluna 'turma' do seu banco
    fotos = conn.execute('SELECT * FROM galeria WHERE turma = ? ORDER BY id DESC', (id_turma,)).fetchall()
    conn.close()
    
    # Enviamos as fotos e o nome da turma para o HTML
...
def upload_foto():
    # Verifica se o representante está logado (usando a chave que definimos no login_rep)
    if 'representante_logado' not in session:
        return redirect(url_for('login_rep'))
    ...
    if file and allowed_file(file.filename):
        ...
        # Cria um nome único para não sobrescrever fotos com o mesmo nome
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S_')
        nome_final = timestamp + filename
        
        # Salva na pasta static/img/uploads/galeria
        caminho_completo = os.path.join(app.config['UPLOAD_FOLDER'], nome_final)
        file.save(caminho_completo)

        # Salva no banco de dados SQLite que você criou
        conn = conectar_banco()
        conn.execute('''
            INSERT INTO galeria (turma, materia, nome_arquivo, data_postagem)
            VALUES (?, ?, ?, ?)
        ''', (turma, materia, nome_final, datetime.now().strftime('%d/%m/%Y %H:%M')))
        conn.commit()
        conn.close()
...
def api_fotos(turma):
    conn = conectar_banco()
    fotos_db = conn.execute('SELECT * FROM galeria WHERE turma = ? ORDER BY id DESC', (turma,)).fetchall()
    conn.close()
    return jsonify([{'materia': f['materia'], 'arquivo': f['nome_arquivo'], 'data': f['data_postagem']} for f in fotos_db])
